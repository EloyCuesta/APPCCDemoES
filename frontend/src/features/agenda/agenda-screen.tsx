...
function AgendaWorkspace({ tenantId, tenantName, refreshAccess }: { tenantId: number; tenantName: string; refreshAccess: () => void }) {
  ...
  return <div className="agenda-screen">
    ...
    <section className="agenda-results" aria-label="Resultados de agenda" aria-busy={state.status === "loading"}>
      ...
      {state.status === "ready" && <><AgendaList items={state.data.items} today={today} /><nav className="agenda-pagination" aria-label="Paginación de agenda"><button type="button" className="button button-secondary" disabled={page === 1} onClick={() => setPage((current) => current - 1)}>Anterior</button><p role="status">Página {page}{total !== null && ` · ${total} controles en la consulta`}</p><button type="button" className="button button-secondary" disabled={!hasNext} onClick={() => setPage((current) => current + 1)}>Siguiente</button></nav></>}
    </section>
  </div>;
}
