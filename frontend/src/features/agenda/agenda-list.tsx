...
export function AgendaList({ items, today }: { items: Item[]; today: string }) {
  ...
  return <>
    ...
    {groups.filter((group) => group.items.length > 0).map((group) => <section className={`agenda-group agenda-group-${group.key}`} aria-labelledby={`group-${group.key}`} key={group.key}>
      ...
      <ul className="agenda-items">{group.items.map((item) => <AgendaItem item={item} key={item.programacion.id} />)}</ul>
    </section>)}
  </>;
}
